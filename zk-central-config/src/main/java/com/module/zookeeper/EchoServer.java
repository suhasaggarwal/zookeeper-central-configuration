...
public class EchoServer extends Thread implements IZkDataListener, IZkStateListener {
  ...
  public EchoServer(int port, int zkPort) {
    this.port = port;
    this.zkClient = new ZkClient("localhost:" + zkPort,1800000,80000);
  ...
  public void run() {
    try {
      ...
      zkLeaderNode = ZkUtils.findLeaderOfNode(nodes, zkNode);
       LOG.info("ZkLeaderNode:" + zkLeaderNode.getPath());
  ...
  private void electNewLeader() {
    ...
    if (zkNode.getSequence().equals(nodes.first().getSequence())) {
      ...
      synchronized (mutex) {
        start.set(true);     //Code to start Opentaps Server/Service
        mutex.notify();
      }
    }
  }
  ...
  public void handleDataDeleted(String dataPath) throws Exception {
    if (dataPath.equals(zkLeaderNode.getPath())) {
      ...
      electNewLeader(); //Elect New leader from Opentaps Quorum when one opentaps server dies - Explore Application in Load Balancing/Multitenancy
    }
  }
}


//How to fit in zookeeper in existing opentaps Architecture
